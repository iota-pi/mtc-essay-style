...
import { SBL_BOOK_MAP, SBL_VALID_ABBREVIATIONS, COMMON_WRONG_ABBREVIATIONS, BIBLE_BOOK_PATTERN, SBL_ABBREV_TO_FULL } from '../data/sbl-books'
import { getParagraphText, findParagraphIndex, getRegionForParagraph, isInsideParentheses, isSentenceStart, getParagraphSnippet } from '../utils'
...
export const bibleBookAbbreviationsRule: StyleRule = {
  ...
  check(context): StyleViolation[] {
    ...
    function getCanonicalNames(rawName: string): { abbrev: string; full: string } | undefined {
      ...
      if (!abbrev) {
        for (const [, fName] of SBL_ABBREV_TO_FULL.entries()) {
