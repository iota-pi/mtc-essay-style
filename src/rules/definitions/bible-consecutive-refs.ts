...
import { BIBLE_BOOK_PATTERN, SBL_BOOK_MAP, COMMON_WRONG_ABBREVIATIONS, SBL_VALID_ABBREVIATIONS } from '../data/sbl-books'
    ...
    function processText(text: string, pIndex: number | undefined, region: DocumentRegion | undefined, footnoteId?: string) {
      ...
      while ((startMatch = CITATION_START_REGEX.exec(text)) !== null) {
        ...
        while (scanIndex < text.length) {
          ...
          match = NEXT_BOOK_REGEX.exec(remaining)
          if (match !== null) {
            matchedType = 'book'
          } else {
            // Try matching next chapter:verse
            match = NEXT_CHAPTER_VERSE_REGEX.exec(remaining)
            if (match !== null) {
              matchedType = 'chapter-verse'
            } else {
              // Try matching next single number
              match = NEXT_NUMBER_ONLY_REGEX.exec(remaining)
              if (match !== null) {
                matchedType = 'number'
              }
            }
