...
import { classifySections } from '../analysis/sections'
import { runChecks, defaultRegistry } from '../rules/index'
  ...
  .action(async (files: string[], options) => {
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (format === 'json') {
          console.info(formatJson(result, resolvedPath))
        } else {
          console.info(formatText(result, resolvedPath, severity))
          console.info('') // Empty line separator
        }
      } catch (e: unknown) {
        const message = e instanceof Error ? e.message : String(e)
        console.error(chalk.red(`Error processing file ${file}: ${message}`))
        hasErrors = true
      }
    }
